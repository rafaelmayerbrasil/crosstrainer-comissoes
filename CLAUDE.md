...
> **✅ ESCALA E MANUAIS — NO AR EM PRODUÇÃO (27/08/2026, `e44b4a7`).** O botão de ajuste do painel de Equilíbrio era um **✏️ mudo** ao lado de "4 nesta janela", e o Rodrigo leu como "editar este número": quis baixar a Heloísa de 4 pra 3, digitou 3 e ela foi pra **4+3=7** — o campo registra **dias trabalhados FORA do sistema**. Virou **"+ dias fora"**, com aviso de que **não muda a escala** e o caminho certo (trocar na vaga ou Refazer). **Os manuais estavam 3 semanas atrasados** — Grade de Horários, prévia, publicar em lote, inverter, cota, desligar pessoa: nada disso estava lá. 13 blocos novos nos dois manuais. `smoke-manual-atualizado.js` (8 casos) ancora os assuntos e caça link morto — **ao entregar recurso que muda a rotina de alguém, acrescentar lá**. Publicado por cherry-pick, sem levar Comissões junto.
>
> > **⏸️ COMISSÕES — ENVIADO AO RODRIGO EM 27/08, ESPERANDO RESPOSTA. NÃO PUBLICAR AINDA.** O upload pela tela está homologado no staging, mas **NÃO em produção** — tudo na branch `comissoes-tradutor-pacto`. ⚠️ Subir o arquivo da Pacto em produção hoje dá **"Nenhum dado encontrado"** (das 549 linhas, zero aproveitadas). **Não publicar** porque (a) duas das respostas do Rodrigo mudam o fechamento de agosto e (b) a **competência ainda vai ser construída** — melhor publicar uma vez só. **Os 3 itens com o Rodrigo:** os **R$ 599,35** de comissão de julho que caiu no vão (30 contratos com início em julho pagos em agosto — Kali 273,45 · Bárbara 163,57 · Francini 90,40 · Erica 71,93; 3 opções na mesa) · confirmar que os **279 contratos migrados** (R$ 78.352) ficam fora · a **meta de agosto** das duas unidades.
>
> **📐 REGRA DE COMPETÊNCIA — decidida e validada, falta construir.** A comissão é do **mês da venda**; paga o que foi **recebido**; corte no **dia 15**; o que entra depois **acumula**. Decisões do Rafael: competência = **`Data Início` do contrato** · corte **dia 15 fixo** · primeira competência **agosto** · **julho fica como está**. **A regra nunca esteve no código** — validei: no histórico de produção todo período só tem vendas do próprio mês, e o "acumula" era feito **re-subindo o mês** (`cp_2026-05` teve 4 uploads convivendo). O corte do dia 15 era o dia em que a pessoa exportava. **Construir é menor do que parece:** não precisa tirar a detecção automática de período, só **trocar o que alimenta ela** (a coluna `Data` passa a levar a competência em vez do recebimento). ❌ **`Data Cadastro` NÃO serve** de data da venda: 85% das linhas trazem `24/07/2026`, o dia da carga da migração — só funciona nos contratos nativos `7xxx` (93% de acerto), e 55 contratos que começam em agosto ainda carregam o carimbo. Revisitar quando os `4xxx` vencerem.
>
> **🔴 CORREÇÃO (26/08):** eu disse que "as metas foram calibradas na época do TecnoFit" e que a Kali e a Bárbara perdiam o P3 por causa da migração. **Errado.** A gestão **define meta mês a mês** (`metasMensais` no doc do período — pp_2026-07 meta 45/minRenov 15; cp_2026-06 meta 55/minRenov 15). O que houve é que **agosto ainda não teve a meta configurada** e o sistema caiu no padrão da unidade. **Sempre checar `metasMensais` antes de concluir qualquer coisa sobre meta** — não olhar só `units/{id}.config`.
>
> **🟢 COMISSÕES — O UPLOAD VOLTOU A SER PELA TELA (26/08/2026), homologado no staging, FALTA MERGE PRO `main`.** O `pacto-adapter.js` foi plugado no `index.html`: arrasta o arquivo da Pacto na tela de Upload e pronto — sem script, sem planilha intermediária. **3 pontos no `index.html`, nada no `commission.js`:** carrega o adapter · o `handleFile` reconhece o arquivo da Pacto, traduz e filtra pela unidade aberta (formato antigo do TecnoFit continua passando direto) · a prévia ganhou o **resumo do que ficou de fora**, por unidade. A tela **barra o "Faturamento por Período"** antes de calcular. Homologado pelo Rafael com o arquivo real: **CP 53 ativações R$ 1.810,11 · PP 27 ativações R$ 722,89**. **Dois bugs meus, os dois pegos por ele no navegador**, mesma raiz — **o id da unidade no sistema (`unit-cp`) não é a sigla do arquivo (`CP`)**: primeiro nenhuma linha era encontrada; depois o resumo aparecia vazio porque eu corrigi o caminho dos dados e **esqueci o da tela**, escondendo R$ 42 mil de descarte. Lição: teste que confere "a função é chamada" não basta — tem que conferir **com o quê**. **⏸️ ABERTO, e é de gestão:** o **P3 do Príncipe zerou** (27 ativações contra mín. 50; 14 novos contra mín. 18, que zera). Não é performance — a base migrada está no meio do contrato e não gera renovação; se corrige sozinho, mas por ora a Kali e a Bárbara pagam a conta da migração. Ajustável pela tela "Configurar Metas do Mês". Branch `comissoes-tradutor-pacto`. Detalhe: `CONTEXTO_SESSAO.md` → sessão 58.
>
