...
> **🔴 FRENTE VIVA — COMISSÕES: FECHAR JULHO.** O Rodrigo respondeu as 16 perguntas em 13/08/2026 e confirmou que **julho nunca foi calculado nem pago** — é o trabalho urgente. Duas decisões anteriores caíram: **(1) "IMPORTAÇÃO não paga comissão" foi REVERTIDA** — as vendas de julho foram feitas no TecnoFit e entraram na Pacto com esse rótulo; o corte certo é por **`Data Início`**. **(2) 🚨 O vendedor de julho se perdeu na migração** — vendas da Erica aparecem como RODRIGO ROJAIS (não-comissionável); agosto é confiável, julho tem que sair dos relatórios das vendedoras. Também: **um mês não cabe num arquivo só** (venda de 24/07 só aparece no export de agosto) e **a data da venda não existe no export**. ⏳ Aguardando os relatórios de julho da Francini, Kali e Bárbara. **Leia `memory/migracao-relatorio-pacto-comissoes.md` antes de tocar em qualquer coisa** — tem as regras confirmadas, o que já está implementado no motor e as armadilhas dos dados. Detalhe: `CONTEXTO_SESSAO.md` → sessão 50. Memórias: [[migracao-relatorio-pacto-comissoes]] · [[comissoes-renovacao-classificada-novo]] · [[fix-split-bianual-recalc]].
>
