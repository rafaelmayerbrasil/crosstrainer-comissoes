...
- **✅ CF `sendEventReminders` DEPLOYADA no staging (12/07):** o billing estava suspenso (cartão vencido) e travou o deploy de Functions por horas com 403 `Write access denied` mesmo com o console verde (lag do Google entre reabrir o billing e reabrir os escopos de escrita p/ Cloud Functions/Build). Após 2º pagamento pendente, propagou. Função **ACTIVE** (GEN_2, Node 22, startup OK, `deployment-scheduled:true` → agendamento diário 9h SP criado). Lógica pura testada (smoke verde). **Falta só o disparo real:** roda sozinha às 9h SP, ou "Executar agora" no Cloud Scheduler → validar logs + `remindersSent` (gcloud não está na máquina; não dá p/ forçar por CLI).
...
**⏭️ RETOMAR AQUI:**
1. **CF ✅ deployada** — falta só ver o disparo real: rodar às 9h SP ou "Executar agora" no Cloud Scheduler → validar logs + `remindersSent` (idempotência). (Opcional: semear evento a 7/4/1 dia com staff pra o próximo ciclo confirmar o envio in-app.)
