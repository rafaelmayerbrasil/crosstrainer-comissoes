...
- [x] **Edição de grade → propagação opt-in CONSTRUÍDA (12/07).** Antes: editar a grade valia só pras datas ainda não geradas (silencioso). Agora: ao salvar a edição de um slot (mesmo dia da semana + campo mudado), oferece um confirm *"Aplicar também às N próximas aulas já criadas?"* → atualiza professor/modalidade/horário **só nas aulas intocadas** (`prevista` + mês aberto + futura); nunca mexe em mês fechado/substituída/passada. Helper puro `class-propagation.js` + smoke, `ClassService.propagateSlotEdit{Plan,Apply}`, hook em `professores-agenda.js`. Client-side, sem CF/índice novo. Spec/plano `2026-07-12-propagacao-edicao-grade*`. Validado E2E no staging. **Ainda vale confirmar a UX com o Rodrigo**, mas o problema (mudança silenciosa) está resolvido. Relacionado (não bloqueia): a "Minha Agenda" do professor só expõe até *próxima semana / mês corrente* (`MINHA_AGENDA_FILTERS`), embora as aulas existam ~4 semanas à frente.
