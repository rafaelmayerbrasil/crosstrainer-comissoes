...
- [x] 6. Consolidado e reportado. Índice CONFIRMADO servindo (query do cliente OK). Fixture limpa. **CHECK COMPLETO — pronto pra revisão final do cliente.**
...
- 🔴 **CORRIGIDO (`ddaf542`):** "Minhas Férias" do professor falhava silenciosamente — índice composto `vacation_requests (teacherId ASC, requestedAt DESC)` exigido pela query NUNCA esteve no `firestore.indexes.json` (criado à mão no console na 6a e perdido). UI mascarava com "Histórico 0". Índice declarado + deployado + **build confirmado** (query do cliente OK em 12/06).
