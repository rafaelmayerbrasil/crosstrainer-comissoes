...
**Versão:** V3 consolidada para validação final  
**Data:** 22/04/2026
...
### 4.0 Contexto: perfis já existentes no app atual

O app atual opera com dois perfis consolidados que continuam válidos e não devem ser alterados:

- **`admin`** — acesso completo ao módulo de comissões e à gestão geral do sistema.
- **`vendedor`** — acesso restrito ao próprio painel de comissões, metas e resultados individuais.

O novo módulo de professores introduz perfis adicionais (`admin_gestao`, `supervisao`, `professor`, `professor_estagiario`) que devem **coexistir dentro da mesma estrutura de usuários**, sem criar um segundo cadastro paralelo.
...
### 4.5 Diretriz de reutilização de perfis e usuários

O novo módulo **não deverá criar um segundo cadastro paralelo de usuários/perfis**.

Deverá ser reaproveitada a estrutura de usuários, autenticação, perfis e permissões já existente no app atual, de forma que a gestão dos acessos continue centralizada em um único ponto do sistema.

O módulo de professores deverá apenas **complementar** essa estrutura com os dados específicos necessários para a operação do novo módulo — como vínculo com professor, modalidade, unidade, tipo de profissional e regras de pagamento — sem duplicar o cadastro principal do usuário.

Sempre que possível, o sistema deve trabalhar com uma **única origem de verdade** para usuário, perfil e permissão de acesso.

### 4.6 Suporte a múltiplos perfis por usuário

O modelo atual de perfil único (`role`) deverá evoluir para suportar **múltiplos perfis por usuário**, permitindo que um mesmo usuário atue em diferentes módulos com papéis distintos.

O modelo proposto é:

- **`profiles[]`** — lista de perfis ativos do usuário (ex: `["admin", "admin_gestao"]`).
- **`moduleAccess{}`** — mapa booleano que indica quais módulos o usuário pode acessar (ex: `{ "comissoes": true, "professores": true }`).

O menu de navegação e as rotas do sistema devem ser gerados **dinamicamente** com base no `moduleAccess{}` do usuário autenticado, exibindo apenas os módulos autorizados.

### 4.7 Segregação de acesso entre módulos

A coexistência dos módulos de comissões e de professores exige separação clara de acesso entre os dois domínios:

- Os perfis `supervisao`, `professor` e `professor_estagiario` **não devem visualizar nem acessar** nenhuma área do módulo de comissões (painéis de vendas, metas, recibos de vendedores, relatórios financeiros de vendas).
- O perfil `vendedor` **não deve visualizar nem acessar** o módulo de agenda/professores (agenda, substituições, escalas especiais, fechamento de professores, pagamentos de professores).
- A segregação deve ser aplicada tanto no frontend (menu dinâmico, supressão de rotas) quanto no backend (regras de segurança por coleção e por perfil autorizado).

### 4.8 Matriz de acesso por módulo

A tabela abaixo consolida quais perfis têm acesso a cada módulo do sistema:

| Perfil                  | Módulo: Comissões | Módulo: Professores / Agenda |
|-------------------------|:-----------------:|:----------------------------:|
| `admin`                 | ✅ completo       | ✅ completo (admin_gestao)   |
| `vendedor`              | ✅ restrito*      | ❌ sem acesso                |
| `admin_gestao`          | ❌ sem acesso     | ✅ completo                  |
| `supervisao`            | ❌ sem acesso     | ✅ operacional (sem salarial)|
| `professor`             | ❌ sem acesso     | ✅ restrito (leitura/próprio)|
| `professor_estagiario`  | ❌ sem acesso     | ✅ restrito (leitura/próprio)|

\* `vendedor`: acesso somente ao próprio painel, metas e resultados de comissão.
...
- **RF27** — reutilizar a estrutura de usuários, autenticação e perfis já existente no app atual, evitando duplicidade de cadastro e mantendo a gestão de acessos centralizada;
- **RF28** — implementar menu de navegação dinâmico, gerado com base no `moduleAccess{}` do usuário autenticado, exibindo apenas os módulos autorizados ao perfil;
- **RF29** — garantir segregação total entre módulos: perfis do módulo de professores não devem acessar o módulo de comissões, e o perfil `vendedor` não deve acessar o módulo de professores/agenda.
...
- **RN20** — o novo módulo não deve criar um segundo cadastro de usuários/perfis; deverá utilizar a base já existente no sistema como origem principal das permissões e acessos;
- **RN21** — o modelo de perfil do sistema deverá suportar múltiplos perfis por usuário (`profiles[]`) e controle de acesso por módulo (`moduleAccess{}`), substituindo progressivamente o campo `role` único;
- **RN22** — os perfis `supervisao`, `professor` e `professor_estagiario` não devem ter acesso visual nem operacional a nenhuma área do módulo de comissões;
- **RN23** — o perfil `vendedor` não deve ter acesso visual nem operacional a nenhuma área do módulo de professores/agenda; o menu e as rotas do sistema devem refletir essa restrição de forma automática.
...
Este documento deverá ser tratado como **base funcional oficial da próxima etapa de desenvolvimento**.
